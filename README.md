...
| **maxRetries**             | number  | `1`         | How many times to retry after the first try (`0` = a single try). GameDig may try multiple ports per server, which multiplies the total queries; set `givenPortOnly` to query just one port.                                        |
