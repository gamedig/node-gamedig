...
function createProtocolInstance(type, timeout) {
    ...
    return new protocol( timeout );
}

class TypeResolver {
    static lookup(type, timeout) {
        ...
        timeout = timeout ? timeout : 1000 // Default timeout for all protocols
        const query = createProtocolInstance(game.protocol, timeout);
