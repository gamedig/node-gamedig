  ...
  async run(state) {
    {
      ...
      state.raw.language = this.decode(reader.uint(1), [
        "any",
        "en",
        "de",
        "fr",
      ...
      state.raw.landscape = this.decode(reader.uint(1), [
        "temperate",
        "arctic",
        "desert",
        "toyland",
  ...
  async query(type, expected, minver, maxver) {
    ...
    return await this.udpSend(b, (buffer) => {
      ...
      if (packetLen !== buffer.length) {
        this.debugLog(
          "Invalid reported packet length: " + packetLen + " " + buffer.length
        );
        return;
      }
      ...
      if (
        (minver !== -1 && protocolVersion < minver) ||
        (maxver !== -1 && protocolVersion > maxver)
      ) {
        throw new Error(
