...
class Doom3 extends Core {
  ...
  async run(state) {
    const body = await this.udpSend(
      "\xff\xffgetInfo\x00PiNGPoNg\x00",
      (packet) => {
    ...
    state.raw.protocolVersion =
      (protoVersion >> 16) + "." + (protoVersion & 0xffff);
    ...
    const isEtqw =
      state.raw.gamename && state.raw.gamename.toLowerCase().includes("etqw");

    const rest = reader.rest();
    let playerResult = this.attemptPlayerParse(
      rest,
      isEtqw,
      false,
      false,
    ...
    if (!playerResult) {
      throw new Error(
        "Unable to find a suitable parse strategy for player list"
      );
    }
    let players;
    ...
    for (const player of players) {
      if (!player.ping || player.typeflag) state.bots.push(player);
