...
const stringKeys = new Set(["website", "gametype", "gamemode", "player"]);
...
class Gamespy1 extends Core {
  ...
  async run(state) {
    ...
    const data = Object.fromEntries(
      Object.entries(raw).map((entry) => normalizeEntry(entry))
    );
    state.raw = data;
  ...
  async sendPacket(type) {
    ...
    return await this.udpSend(type, (buffer) => {
      ...
      if (!partNum) {
        partNum = parts.size;
        this.logger.debug(
          "No part number received (assigned #" + partNum + ")"
        );
      }
      ...
      this.logger.debug(
        "Received part #" + partNum + " of " + (maxPartNum ? maxPartNum : "?")
      );
